...
class feature_extract:
    def __init__(self, wav_file, ssl_model):
        self.wav = wav_file
        ...
        #
        # self.audio_len = np.int(self.MAXLEN * self.sampling_rate)

    def run_extraction(self):
        if self.ssl_model == 'hubert':
            model_hub_hubert = "facebook/hubert-large-ll60k"
            model_extractor = HuggingFaceWav2Vec2(model_hub_hubert, save_path='')
        elif self.ssl_model == 'wavlm':
            model_hub_wavlm = "microsoft/wavlm-large"
            model_extractor = HuggingFaceWav2Vec2(model_hub_wavlm, save_path='')

        source_ar, sr = librosa.load(self.wav, sr=16000)
        audio_len = np.int(self.MAXLEN * sr)
        ...
        if len(source_ar) <= audio_len:
            no_segs = 1
            spk_wav = np.concatenate([source_ar, np.zeros(audio_len - len(source_ar))])
